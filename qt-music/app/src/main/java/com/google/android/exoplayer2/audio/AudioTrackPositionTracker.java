...
final class AudioTrackPositionTracker {
    ...
    public boolean hasPendingData(long j) {
        return j > durationUsToFrames(getCurrentPositionUs(false)) || forceHasPendingData();
    }
    ...
    private long getPlaybackHeadPosition() {
        ...
        if (j != -9223372036854775807L) {
            return Math.min(this.endPlaybackHeadPosition, this.stopPlaybackHeadPosition + durationUsToFrames(Util.getMediaDurationForPlayoutDuration((jElapsedRealtime * 1000) - j, this.audioTrackPlaybackSpeed)));
        }
