...
class HlsChunkSource {
    ...
    public void getNextChunk(long j, long j2, List<HlsMediaChunk> list, boolean z, HlsChunkHolder hlsChunkHolder) {
        ...
        Uri fullEncryptionKeyUri = getFullEncryptionKeyUri(hlsMediaPlaylist3, segmentBaseHolder.segmentBase.initializationSegment);
        hlsChunkHolder.chunk = maybeCreateEncryptionChunkFor(fullEncryptionKeyUri, i);
        if (hlsChunkHolder.chunk != null) {
            return;
        }
        Uri fullEncryptionKeyUri2 = getFullEncryptionKeyUri(hlsMediaPlaylist3, segmentBaseHolder.segmentBase);
        hlsChunkHolder.chunk = maybeCreateEncryptionChunkFor(fullEncryptionKeyUri2, i);
        if (hlsChunkHolder.chunk != null) {
            return;
        }
        Uri uri4 = uri;
        boolean zShouldSpliceIn = HlsMediaChunk.shouldSpliceIn(hlsMediaChunk, uri, hlsMediaPlaylist3, segmentBaseHolder, initialStartTimeUs);
        if (zShouldSpliceIn && segmentBaseHolder.isPreload) {
            return;
        }
        CmcdConfiguration cmcdConfiguration = this.cmcdConfiguration;
        hlsChunkHolder.chunk = HlsMediaChunk.createInstance(this.extractorFactory, this.mediaDataSource, this.playlistFormats[i], initialStartTimeUs, hlsMediaPlaylist3, segmentBaseHolder, uri4, this.muxedCaptionFormats, this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), this.isPrimaryTimestampSource, this.timestampAdjusterProvider, this.timestampAdjusterInitializationTimeoutMs, hlsMediaChunk, this.keyCache.get(fullEncryptionKeyUri2), this.keyCache.get(fullEncryptionKeyUri), zShouldSpliceIn, this.playerId, cmcdConfiguration != null ? CmcdLog.createInstance(cmcdConfiguration, this.trackSelection, j, j2) : null);
    }
